...
{ 
	...
	if ( HWREV == 0)
	{
	reg = (readl(dsim_base + 0x58)) & ~(1 << 0x01);

	reg |= enable << 0x01;

	writel(reg, dsim_base + 0x58);
	}
//hs_clock 
	reg = (readl(dsim_base + S5P_DSIM_CLKCTRL)) &
...
unsigned int s5p_dsim_is_interrupt_status(unsigned int dsim_base)
{
...
{	
	u32 val;
	void __iomem *regs = ioremap(S5PV210_PA_LCD, SZ_4K);
