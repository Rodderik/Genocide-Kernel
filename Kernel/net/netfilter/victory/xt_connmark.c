...
static bool connmark_mt(const struct sk_buff *skb, const struct xt_match_param *par)
{
	...
	if (ct == NULL)
        ...
        if(((ct->mark & info->mask) == info->mark) ^ info->invert)
	return true;
        else 
         return false;
}

static bool connmark_mt_v0(const struct sk_buff *skb, const struct xt_match_param *par)
{
